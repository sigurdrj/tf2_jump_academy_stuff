...
#define S 15 // Waypoint box size (half)
...
int main(int argc, char *argv[]){
	...
	for (auto l : layout){
		for (auto waypoint : l["waypoints"]){
			if (!waypoint["identifier"].is_null()){
				// Get coordinates from identifier
				auto c = get_entity_origin_by_name(mapEntities, waypoint["identifier"], "info_");

				std::cout << c.x << ' ' << c.y << ' ' << c.z << '\n';
				scriptFile << "box " << c.x-S << ' ' << c.y-S << ' ' << c.z-S << ' ' << c.x+S << ' ' << c.y+S << ' ' << c.z+S << '\n';
			...
			std::cout << x << ' ' << y << ' ' << z << '\n';
			scriptFile << "box " << x-S << ' ' << y-S << ' ' << z-S << ' ' << x+S << ' ' << y+S << ' ' << z+S << '\n';
		}

		auto controlpoint = l["controlpoint"];
		if (!controlpoint["identifier"].is_null()){
			// Get coordinates from identifier
